...
macOS 的机器合同见 [`macos-adaptation-contract.toml`](macos-adaptation-contract.toml)。它只记录三类长期事实：支持矩阵、安全不变量，以及 portable 与原生证据的区别。
...
构建与安装必须继续拒绝错误架构、非标准 CPython ABI、未归属 staging、符号链接 payload 和未验证的 purge 路径。正式产物使用原子发布；quarantine 与 payload inventory 校验 fail closed。Camoufox 浏览器不打入离线包，安装过程不下载浏览器，受管 runtime 只能在显式准备或既有 auto-prepare 边界内获取。

## 证据等级

- `scripts/validate_macos_adaptation.py` 从项目元数据、workflow、installer manifest 和 release asset owner 读取当前事实并校验合同。
- Windows 与 Linux/WSL 入口提供 portable 静态和纯 Python 证据，不证明 Mach-O、codesign、quarantine、文件模式、大小写或原生浏览器 bundle。
