...
2. 在 `src/paper_fetch/providers/` 添加 provider owner，并声明完整 `ProviderBundle`；`client_factory` 必须直接持有 client 类或同模块 typed callable，不使用字符串反射。优先复用现有 resolver、HTTP policy、HTML/JATS/PDF 提取、browser facade、asset budget 和 acceptance owner。
