...
Camoufox/Playwright 的 navigation、redirect、子资源与 service worker 使用浏览器原生行为；项目不安装 BrowserContext-wide URL/DNS interceptor，也不把带 cookie、storage state、profile 或 user-data 的 context 绑定到单一 origin。Provider image/font/media 优化只在 page scope 按资源类型生效，external CDP 可继续借用既有 context。Browser 发现的二进制 URL 仍交给受控 direct transport，由 `SafeRemoteUrlPolicy` 与 catalog host policy 在读取时逐跳验证；Browser cookie 转 direct opener 使用标准 `CookieJar`/`DefaultCookiePolicy`，保留 host-only/domain、path、secure 和 expiry 语义。
