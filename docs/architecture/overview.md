...
`compile_route_execution_policy()` 是 catalog 到 runtime 的唯一编译边界。它把 exact/suffix/base domain、API/CDN/template/route host 合并成同一 allowlist，并把 timeout、transient/rate retry、QPS/minimum interval、rate-wait budget、acceptance policy、asset scope 与 route concurrency cap 投影到 HTTP、browser、PDF 和 asset runtime；MCP provider-catalog resource 同时公开原声明与 `execution_policy`。minimum interval 通过每 scope 串行 start gate 执行，排队和 cooldown 等待不占 host semaphore；迟到的 `Retry-After` 移动队首后，后续请求仍从实际起点继续保持间隔。未显式提供 asset profile 时，route `asset_scope` 选择执行范围；显式 `none|body|all` 保持用户覆盖。Acceptance 分别用 resolved identity、匹配 representation 的 fulltext 或已审计/本地 asset 事实验证声明，未知 policy 不会被当作满足。Registry 在注册时统一检测规范化 alias、DOI prefix、exact/suffix domain 及交叉覆盖；只有双方声明不同 `identity_priority` 和非空原因时才允许有意重叠，identity lookup 按该优先级而不是导入顺序决定。
...
- Browser 资产链只负责解析、URL 发现和 cookie/storage-state 授权，不读取远端二进制 body。可重放 URL 交给 verified-IP pinned `HttpTransport.stream_to_file()`，以目标同目录唯一 `xb` staging、flush/fsync 和原子发布完成；不能转成受策略约束的 direct stream 时返回 `browser_stream_unavailable`。EPS/TIFF、PDF screenshot copy 与 arXiv source figure 采用 path-to-path 处理，输出继续受同一字节/像素预算约束。
- 资产 future 以 `as_completed` 顺序立即保存或释放 staging/reservation，最终只把轻量结果恢复为输入顺序。致命文件/字节/像素超限会保留首个稳定 reason、删除所有登记 staging、设置 cooperative stop fence 并取消 pending future；来自 `RuntimeContext` 的外部取消不会被内部 budget stop 合并掉。Runtime-scoped session pool 为每个 worker 建独立标准 `CookieJar`，通过 pinned transport 有界 GET seed 一次，并跨 figure/supplementary phase 复用 ready 状态；redirect 每跳导入 cookie 后为下一跳刷新，provider catalog 的敏感 header 策略仍控制跨源剥离。
...
`HttpTransport` 保持 public request options、structured logs、cancel checks、`Retry-After` 最大等待和 `RequestFailure` 形状；瞬时错误与 429 retry policy 由 `urllib3.util.Retry` 表达，连接池由 `PoolManager(num_pools, maxsize, block=True)` 配置。`SafeRemoteUrlPolicy` 的公网 DNS 结果直接成为 urllib3 TCP endpoint，原 publisher host 只用于 HTTP `Host`、TLS SNI 与证书 hostname 校验，因此校验后不会再用 hostname 做第二次 DNS lookup；每个 redirect hop 都重新解析并验证。`HttpRequestPolicy.sensitive_headers` 由 provider catalog request policy 自动注入，自定义 API/TDM 凭据和标准认证 header 在跨 origin redirect 时移除，实际发送 catalog 凭据的 route 必须有 host allowlist。磁盘 textual GET 缓存使用脱敏 cache key（敏感 header 用短 SHA-256 digest 区分凭据且不落原文），默认按 `4096` 条、`512 MiB`、`30` 天清理，三项上限可用环境变量独立覆盖。内部子模块：`transport.py`（request loop / pool / semaphore / log）、`url_policy.py`（URL、DNS/IP 与 redirect 安全）、`provider_policy.py`（catalog 到 request policy）、`browser_policy.py`（browser request guard）、`cache.py`（cache key / digest / memory+disk cache / stats / prune）、`retry.py`（retry policy / backoff）、`body.py`（读取 / 解压 / content-type / preview）、`errors.py`（异常类型）。`paper_fetch.http` 是兼容 facade。
...
Camoufox/Playwright 的 navigation、request-context GET、redirect 和最终 URL 复用同一安全策略与 catalog host allowlist；BrowserContext-wide route interceptor 在真正发包前校验 DNS/IP。新 context 显式阻断 service worker，并在 cookie seed/page 创建前安装 interceptor；不能应用这些选项的 external borrowed context 改建隔离 context，安装失败则 fail closed。带 cookie、storage state、profile 或 user-data 的 context 绑定认证 origin，跨 origin 资产不进入该浏览器 context，而回退到受控 direct transport；无法观察 redirect hop 的跨源页面内 fetch 禁用。Browser cookie 转 direct opener 使用标准 `CookieJar`/`DefaultCookiePolicy`，保留 host-only/domain、path、secure 和 expiry 语义。
...
`.github/workflows/ci.yml` 是普通 `push` / `pull_request` 的薄触发器，完整命令事实来源是普通 CI 与 stable release 共用的 reusable `.github/workflows/verify.yml`。它在调用者给出的不可变 commit SHA 上运行完整 unit branch coverage、integration、devtools、Ruff、完整生产包 mypy、复杂度、provider route/catalog/manifest/fixture/docs 治理、抽取规则、版本与依赖漏洞门禁，并把 140 个可执行 exact fixture 按 provider 稳定分成四个 shard，每个 fixture 恰好运行一次；Python 3.11 与 3.14 还分别执行 core unit boundary 和 core/full wheel smoke。wheel/sdist 的完整 archive（不仅 `paper_fetch` 前缀）按结构化归一化后的唯一 root、`dist-info`/`egg-info`、metadata/`RECORD`、声明 data-files 和 static skill 做精确 inventory，任何未知 top-level、`.data`、源码或 metadata member 都失败；两种分发物分别进入隔离 venv 执行 CLI/import/MCP/resource smoke。`quality/coverage-focus.json` 是风险区域纯分支整数基线；`report_coverage_focus.py` 逐文件聚合 coverage.py 官方 `Coverage.branch_stats()` 返回的 taken/total branch exits，同时报告 covered/total、精确百分比及其整数 floor，include 未匹配、文件未测量或区域无分支都会失败。安全边界聚合门槛固定为 90%，其余区域按完整 unit 实测纯分支 floor 固化并禁止下降。provider governance 复用生产测试 corpus loader，只有 canonical raw + expected contract + 当前可执行 builder 的 real replay 才覆盖 route；synthetic/unit/manifest-only 只报告不顶替。runtime 快照与自动路由文档由 `scripts/check_provider_governance.py --update` 生成，正常 CI 只校验、不自动改文件。CI 与本地 unit / integration / devtools 默认复用 `pyproject.toml` 的 `pytest-xdist` 并行配置，不传 `-n 0`。稳定 release 先 peel tag 到完整 commit SHA 并复用上述 verify gate，再在同一 run 冻结九目标依赖；每个 offline staging 的实际 Python、Node/browser、formula/image/native 与 embedded runtime dependency manifest/CycloneDX SBOM 随目标 artifact 发布。发布 job 对 31 个预 checksum 输入做 exact set/basename collision 检查，排他复制成 GitHub Release 的扁平命名空间，再生成 basename-only `SHA256SUMS`。真实 publisher/MCP/browser/auth live 仍只允许显式入口；独立 `provider-canary.yml` 只定时运行无凭据 public direct routes，单次网络失败不阻断，按 route 连续三次失败才发 warning，成功会清零状态。只有 live provider、共享真实 publisher/API 状态、Windows 最终 EXE 安装状态或专门排查顺序问题的测试可串行，并在命令旁说明原因。
