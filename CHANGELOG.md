...
- Adopted the refreshed compatible dependency lock, including MCP 2.1.1 and imagesize 2.0.1, and repaired its validation gate: tool payload tests validate the advertised Draft 2020-12 JSON Schema instead of an SDK-internal generated model, cache authorization tests preserve fail-closed behavior across SDK exception wrapping or propagation, and synthetic PNG fixtures now carry the required IHDR fields.
