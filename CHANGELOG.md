...
- Preserved the first stable challenge/paywall/access-boundary reason when the conservative browser retry exhausts the shared deadline, and kept that `no_access` classification through provider-owned PDF waterfalls. Live acceptance now skips only machine-readable legal-access boundaries while leaving extraction, empty-shell, and incomplete-body regressions as failures.
...
- Authenticated CI/live Camoufox release discovery with the workflow's read-only GitHub token, preventing native macOS and hosted live runtime preparation from failing under GitHub's anonymous API rate limit.
