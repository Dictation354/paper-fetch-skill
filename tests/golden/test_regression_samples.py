...
from tests.provider_benchmark_samples import WILEY_PDF_FALLBACK_SAMPLE
import unittest
...
NATURE_HTML_SAMPLES = [
    {
        ...
        "figure_caption_contains": "Checking for holes in mosquito netting",
    },
    {
        ...
        "figure_caption_contains": "Children at an open-air school",
    },
    {
        ...
        "figure_caption_contains": "The effect of ozone depletion on the surface climate",
    },
]
...
class RegressionSampleTests(unittest.TestCase):
    ...
    def _bilingual_case_specs(self):
        return {
            "wiley": {
                ...
                "source_fixture": "golden_criteria/10.1111_gcb.16386/bilingual.html",
                "abstract_selector": "section.article-section__abstract",
                "abstract_headings": ["Abstract", "Resumo"],
                "first_body_heading": "1 INTRODUCTION",
            },
            "springer": {
                "builder": self._build_springer_bilingual_fixture_article,
                "source_fixture": "golden_criteria/10.1007_s13158-025-00473-x/bilingual.html",
                "abstract_selector": "section[aria-labelledby^='Abs']",
                "abstract_headings": ["Abstract", "Résumé", "Resumen"],
    ...
    def _assert_bilingual_fixture_case(self, case_name: str) -> None:
        ...
        if "source_fixture" in case:
            ...
            source = BeautifulSoup(
                read_fixture_bytes(case["source_fixture"]), "html.parser"
            )
            originals = source.select(case["abstract_selector"])
            abstracts = [s for s in article.sections if s.kind == "abstract"]
            self.assertEqual(len(originals), len(abstracts))
            for original, abstract in zip(originals, abstracts, strict=True):
                paragraphs = original.select("p")
                self.assertTrue(paragraphs)
                for paragraph in paragraphs:
                    self.assertIn(
                        _words(paragraph.get_text(" ", strip=True)),
                        _words(abstract.text),
                    )
    ...
    def _build_springer_bilingual_fixture_article(self):
        return build_shared_html_fixture_article(
            ...
            metadata={
                ...
                "title": "Stagnated Development of Home Language Vocabulary in Japanese–Chinese Bilingual Children at Risk for Autism Spectrum Disorder",
                "journal_title": "International Journal of Early Childhood",
            },
            noise_profile="springer_nature",
        )

    def _build_elsevier_bilingual_fixture_article(self):
        fixture_name = "golden_criteria/10.1016_j.edumed.2018.09.002/original.xml"
        landing_url = (
            "https://www.sciencedirect.com/science/article/pii/S1575181318302614"
        )
        metadata = {
            "doi": "10.1016/j.edumed.2018.09.002",
            "title": "Simulación de la cesárea perimortem, más allá del «saber qué»",
            "landing_page_url": landing_url,
        }
        raw_payload = RawFulltextPayload(
            provider="elsevier",
    ...
    def test_elsevier_bilingual_fixture_preserves_parallel_abstract_sections(
        ...
        article = self._build_elsevier_bilingual_fixture_article()
        source = BeautifulSoup(
            read_fixture_bytes(
                "golden_criteria/10.1016_j.edumed.2018.09.002/original.xml"
            ),
            "xml",
        )
        abstracts = source.find_all("abstract")
        self.assertEqual(len(abstracts), 2)
        self.assertEqual(abstracts[1].get("xml:lang"), "en")
        self.assertEqual(
            [len(abstract.find_all("simple-para")) for abstract in abstracts], [4, 4]
        )
        rendered = [
            section for section in article.sections if section.kind == "abstract"
        ]
        # Independent original paragraphs, including the English parallel
        # abstract. A synthetic fixture previously hid this omission.
        expected = []
        for abstract in abstracts:
            for subsection in abstract.find_all("abstract-sec"):
                heading = subsection.find("section-title")
                expected.append(
                    (
                        heading.get_text(" ", strip=True) if heading else "Abstract",
                        _words(
                            subsection.find("simple-para").get_text(" ", strip=True)
                        ),
                    )
                )
        self.assertEqual(
            [(section.heading, _words(section.text)) for section in rendered], expected
        )
        self.assertEqual(article.metadata.abstract, rendered[0].text)
    ...
    def test_wiley_derived_markdown_pdf_assembly_contract(
        self,
    ) -> None:
    ...
    def test_injected_elsevier_failure_falls_back_to_crossref_metadata(
        self,
    ) -> None:
