...
class HtmlSemanticsTests(unittest.TestCase):
    def test_heading_category_maps_canonical_headings(self) -> None:
        ...
        self.assertEqual(
            heading_category("h2", "Data availability statement"), "data_availability"
        )
        self.assertEqual(
            heading_category("h2", "Code availability"), "code_availability"
        )
        self.assertEqual(
            heading_category("h2", "Software availability statement"),
            "code_availability",
        )
        self.assertEqual(
            heading_category("h2", "Data, code, and materials availability"),
            "data_availability",
        )
        self.assertEqual(
            heading_category("h2", "References"), "references_or_back_matter"
        )
        back_matter_fixture_headings = (
            (
                golden_criteria_asset("10.1038/nature13376", "original.html"),
                "Acknowledgements",
            ),
            (
            ...
            (
                golden_criteria_asset("10.1126/sciadv.abg9690", "original.html"),
                "Statement of Competing Interests",
            ),
            (
                block_asset("10.1007/s00382-018-4286-0", "raw.html"),
        ...
        for path, phrase in back_matter_fixture_headings:
            ...
            with self.subTest(heading=text):
                self.assertEqual(
                    heading_category(tag_name, text), "references_or_back_matter"
                )
        ...
        for path, phrase in (
            ...
            (
                golden_criteria_asset("10.1007/s10584-011-0143-4", "article.html"),
                "Open Access",
            ),
        ):
            tag_name, text = _fixture_heading(path, phrase)
...
""",
    ...
    def test_markdown_heading_taxonomy_maps_article_sections(self) -> None:
        self.assertEqual(
            parse_markdown_heading("### Data Availability"), (3, "Data Availability")
        )
        ...
        self.assertEqual(
            markdown_heading_category("Data Availability"), "data_availability"
        )
        self.assertEqual(
            markdown_heading_category("Code Availability"), "code_availability"
        )
        self.assertEqual(
            markdown_heading_category("Data, Materials, and Software Availability"),
            "data_availability",
        )
        self.assertEqual(
            markdown_heading_category("References"), "references_or_back_matter"
        )
        for path, phrase in (
            (
                golden_criteria_asset("10.1038/nature13376", "original.html"),
            ...
            with self.subTest(heading=text):
                self.assertEqual(
                    markdown_heading_category(text), "references_or_back_matter"
                )
        self.assertEqual(
            markdown_heading_category("Rights and permissions"), "auxiliary"
        )
        for path, phrase in (
            (
                golden_criteria_asset("10.1126/sciadv.abg9690", "original.html"),
