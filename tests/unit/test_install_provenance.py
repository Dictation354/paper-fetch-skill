...
def _context(
    ...
    home: Path,
    *,
    distribution_version: str = "5.0.1",
    cli_version: str = "5.0.1",
...
def test_release_version_sources_are_synchronized() -> None:
    ...
    assert f"## {version} - 2026-08-04" in (REPO_ROOT / "CHANGELOG.md").read_text(
        encoding="utf-8"
    )
    assert f"## {version} - 2026-08-04" in (REPO_ROOT / "CHANGELOG_CN.md").read_text(
        encoding="utf-8"
    )
