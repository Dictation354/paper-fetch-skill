...
- 对 runtime `capabilities.browser_available=true`（由 provider routes 派生） 的 provider，在首次联网抓取前先用 `provider_status(provider=...)` 确认静态 runtime；需要真实链路证明时再运行 MCP `browser_preflight(provider=..., browser_auto_prepare=true)` 或 CLI `paper-fetch browser-preflight --provider ...`。CLI 默认允许 managed runtime 首次按需准备，MCP 默认关闭，只有任务确需 browser 且允许联网/本地 runtime 写入时才显式开启。live preflight 可能联网并写 storage-state，不能当作只读检查。
