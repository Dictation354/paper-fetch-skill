...
- 当前十个工具在 `tools/list` 中的 `output_schema` 都为空，不发布协议级 `outputSchema`。`CallToolResult.structured_content` 继续返回带 `schema_version=2` 的版本化 payload；调用方按工具文档和实际字段消费，不把它误称为 output schema。
