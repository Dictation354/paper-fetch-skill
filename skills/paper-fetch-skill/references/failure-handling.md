...
| browser transient：trace/preflight 出现 `browser_runtime_prepare_failed`、`browser_runtime_prepare_timeout`、`browser_runtime_repair_failed`、`browser_connect_timeout`、`cdp_connect_failed`、`browser_context_create_failed`、`browser_page_create_failed` 或 `camoufox_request_failed`，且不含确定性 challenge/auth 证据 | 先读取 `backend/code/stage/runtime_state/version/output_tail/diagnostic_path` 并检查静态配置；`provider_status()` 不是 live 健康证明。MCP 若策略关闭且任务允许联网准备，可把单次请求改为 `browser_auto_prepare=true`，CLI 可去掉 `--no-browser-auto-prepare`。只有 browser profile/CDP endpoint、准备策略、认证状态、运行时健康、请求参数或环境发生变化才重试 | 无状态变化、preflight 为 challenge/auth_required/runtime_error 且未解决、相同失败重复，或达到 attempt 3；`browser_runtime_prepare_cancelled` 不自动重试 | provider、backend、精确 browser code/stage、runtime 状态/版本、有界输出、diagnostic artifact、preflight/trace、变化项、`source_trail`、attempt |
