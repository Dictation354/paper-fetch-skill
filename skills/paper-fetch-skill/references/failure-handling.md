...
- For `wiley` / `science` / `pnas` / `annualreviews` / `royalsocietypublishing` / `acs` / `iop` / `aip` / `mdpi`, the main browser HTML asset path should only emit `full_size` or `preview`; if challenge recovery fails, inspect `quality.asset_failures[*].reason` and `recovery_attempts` rather than expecting a separate canvas tier. AMS direct HTTP figure conversion may emit `source_converted` when EPS/TIFF source figures are converted successfully.
