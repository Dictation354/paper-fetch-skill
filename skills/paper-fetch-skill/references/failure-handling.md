...
| 确定性 `no_result` / `not_supported` / 解析错误：合法响应在同一输入和路线下稳定缺少标识、正文或受支持格式 | 只有规范 identity、所需 mode、合法 provider route 或输入内容确实变化时才重试；泛化的 `no_result` 先结合 trace 判断是否应转入网络/browser 行 | 没有新的输入/路线、相同解析证据重复，或达到 attempt 3 | `code`、规范 identity、provider/route、`source_trail`、确定性证据、attempt |
...
| browser transient：trace/preflight 出现 `managed_chrome_exited_before_cdp`、`managed_chrome_cdp_timeout`、`cdp_connect_failed`、`browser_context_create_failed`、`browser_page_create_failed` 或 `camoufox_request_failed`，且不含确定性 challenge/auth 证据 | 先读取 `backend/code/stage/exit_code/stderr_summary/diagnostic_path` 并检查静态配置；在执行面可用时做 live browser preflight，`provider_status()` 不是 live 健康证明。CloakBrowser runtime 已在单次调用内对确认 stale 的 singleton 至多恢复并重启一次；只有 browser profile/CDP endpoint、backend、认证状态、运行时健康、请求参数或环境发生变化才重试 | 无状态变化、preflight 为 challenge/auth_required/runtime_error 且未解决、相同失败重复，或达到 attempt 3 | provider、backend、精确 browser code/stage、脱敏摘要、diagnostic artifact、preflight/trace、变化项、`source_trail`、attempt |
