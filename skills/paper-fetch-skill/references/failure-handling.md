...
| 确定性 `no_result` / `not_supported` / `response_too_large` / `unsupported_url_scheme` / `unsafe_redirect` / 解析错误：合法响应在同一输入和路线下稳定缺少标识、正文或受支持格式，或请求违反下载安全边界 | 只有规范 identity、所需 mode、合法 provider route、明确缩小的响应范围或输入内容确实变化时才重试；不得放宽响应上限、scheme 或重定向安全校验来重试 | 没有新的输入/路线、相同解析或安全边界证据重复，或达到 attempt 3 | `code/error_category`、规范 identity、provider/route、`source_trail`、确定性证据、attempt |
