...
def metadata_matches_leading_abstract_context(
    metadata_abstract: str, abstract_sections: Sequence[Any]
) -> bool:
...
def resolve_issue_primary_abstract(article: Any) -> str:
    ...
    metadata_abstract = normalize_issue_section_text(
        getattr(metadata, "abstract", None)
    )
    if metadata_abstract and not metadata_matches_leading_abstract_context(
        metadata_abstract, abstract_candidates
    ):
...
def collect_issue_flags(
    provider: str, envelope: FetchEnvelope, *, status: str
) -> list[str]:
    ...
    body_sections = (
        filtered_body_sections(getattr(article, "sections", []) or [])
    ...
    if abstract_text and status == FULLTEXT:
        ...
        overlap_detected = bool(body_text) and has_abstract_body_overlap(
    ...
    if article is not None and any(
        reference_doi_requires_normalization(item.doi) for item in article.references
    ):
        issue_flags.append("refs_doi_not_normalized")
    if (
        status == FULLTEXT
        and envelope.source
        not in EXPECTED_FULLTEXT_SOURCES_BY_PROVIDER.get(provider, frozenset())
    ):
        issue_flags.append("unexpected_source_path")
    if (
        article is not None
        and not getattr(metadata, "authors", [])
        and not is_authorless_briefing_like(
            article,
            provider=provider,
        )
    ):
...
def has_inflated_abstract(
    ...
    *,
    ...
    if (
        abstract_tokens >= ABSTRACT_INFLATED_TOKEN_THRESHOLD
        or len(abstract_text) >= ABSTRACT_INFLATED_CHAR_THRESHOLD
    ):
        return True
...
def is_authorless_briefing_like(article: Any, *, provider: str | None = None) -> bool:
    ...
    return any(
        all(item in heading_set for item in signature) for signature in signatures
    )
